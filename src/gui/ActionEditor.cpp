...
ActionEditor::ActionEditor(QWidget *parent) : QWidget(parent)
{
    ...
    connect(m_actionsList, &QListWidget::currentRowChanged, 
            this, &ActionEditor::onActionSelected);
    ...
    m_actionType->addItems({"Comando Shell", "Script Externo", "Modificar Widget", "Consultar Widget", "Callback"});
    ...
    connect(helpBtn, &QPushButton::clicked, this, [this]() {
        QMessageBox::information(this, "Variáveis Disponíveis",
...
void ActionEditor::saveActionsToWidget()
{
    ...
    QJsonDocument doc(m_actionsData);
    m_target->setProperty("showbox_actions", QString(doc.toJson(QJsonDocument::Compact)));
...
void ActionEditor::updateActionsList()
{
    ...
    for (const QJsonValue &val : actions) {
        ...
        if (type == "shell") {
            ...
            desc = QString("✏️ %1.%2 = %3").arg(action["target"].toString(), 
                                                  action["property"].toString(),
                                                  action["value"].toString());
        } else if (type == "query") {
            desc = QString("❓ %1 → $%2").arg(action["target"].toString(),
                                              action["variable"].toString());
...
{
    int row = m_actionsList->currentRow();
    if (row < 0 || row >= m_actionsList->count() - 1) return;
...
void ActionEditor::onActionSelected(int row)
{
    ...
    int typeIndex = 0;
    if (type == "shell") typeIndex = 0;
    else if (type == "script") typeIndex = 1;
    else if (type == "set") typeIndex = 2;
    else if (type == "query") typeIndex = 3;
    else if (type == "callback") typeIndex = 4;
...
void ActionEditor::onActionTypeChanged(int index)
{
    ...
    if (index == 0) m_commandLine->setPlaceholderText("ex: notify-send \"Clicado!\"");
    else if (index == 1) m_commandLine->setPlaceholderText("ex: ./scripts/handler.sh");
    else if (index == 4) m_commandLine->setPlaceholderText("ex: button_clicked");
...
void ActionEditor::onTestAction()
{
    ...
    if (type == "shell" || type == "script") {
        ...
        if (level == Dangerous) {
            int ret = QMessageBox::critical(this, "⚠️ Comando Perigoso",
            ...
            int ret = QMessageBox::warning(this, "⚠️ Aviso de Segurança",
...
void ActionEditor::updateSecurityIndicator()
{
    ...
    switch (level) {
        case Safe:
            ...
            m_securityLabel->setStyleSheet("background-color: #1a4d1a; color: #90EE90; padding: 5px; border-radius: 3px;");
            break;
        case Warning:
            m_securityLabel->setText("⚠️ " + getSecurityWarning(cmd).split("\n").first());
            m_securityLabel->setStyleSheet("background-color: #4d4d1a; color: #FFD700; padding: 5px; border-radius: 3px;");
            break;
        case Dangerous:
            m_securityLabel->setText("❌ PERIGOSO: " + getSecurityWarning(cmd).split("\n").first());
            m_securityLabel->setStyleSheet("background-color: #4d1a1a; color: #FF6B6B; padding: 5px; border-radius: 3px;");
            break;
    }
